...
<head>
    ...
    <style>
        ...
        .logo { font-size: 24px; font-weight: bold; text-decoration: none; display: flex; }
        ...
        .search-form { display: flex; align-items: center; flex-grow: 1; max-width: 800px; gap: 10px; }
        ...
        .button-group button:hover { border: 1px solid #dadce0; background: #f1f3f4; }

        /* Results Styling */
        .main-content { padding: 20px 50px; max-width: 1000px; }
        .card { border: 1px solid #ddd; padding: 25px; margin-bottom: 20px; border-radius: 8px; }
        .keyword-pill { background:#f1f3f4; padding:3px 8px; margin-right:5px; border-radius:12px; font-size: 16px; display: inline-block; margin-top: 5px; }
        .url-link { color: #006621; text-decoration: none; font-size: 18px; word-break: break-all; }
    </style>
</head>
<body>
...
<div class="top-bar">
    ...
    <form class="search-form" id="searchForm" action="query.jsp" method="get">
        <div class="search-input-container">
            <svg focusable="false" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" fill="#9aa0a6">
...
<% if (query != null && !query.trim().isEmpty()) { %>
    <div style="color: #70757a; font-size: 1.1em; margin-bottom: 20px;">
        ...
        — <strong>Mode: <%= isExact ? "Exact Phrase" : "Fuzzy" %></strong>
    </div>
<% } %>
...
<%
    if (result != null && !result.isEmpty()) {
        ...
        while (!result.isEmpty() && i < 50) {
            ...
            String label = "";
            String color = "";
            if (score >= 0.8) { label = "Very Relevant 🥰"; color = "#28a745"; }
            else if (score >= 0.5) { label = "Relevant 😄"; color = "#17a2b8"; }
            else if (score >= 0.3) { label = "Low Relevant 😙"; color = "#ffc107"; }
            else if (score > 0) { label = "Very Low Relevant 🤨"; color = "#fd7e14"; }
            else { score = 0.0; label = "Not Relevant 😑"; color = "#dc3545"; }
...
<div class="card">
    <div>
        ...
        <a href="<%= doc.url() %>" style="font-weight: bold; color: #1a0dab; text-decoration: none; font-size: 24px;">
            <%= (doc.title() == null || doc.title().isEmpty()) ? "Untitled" : doc.title() %>
        </a>
    </div>

    <p style="margin: 5px 0;">
        <a href="<%= doc.url() %>" class="url-link"><%= doc.url() %></a>
    </p>
    ...
    <p style="font-size: 16px; color: #555;">
        Last mod: <%= doc.lastModificationTimestamp().toString().replace("T", " ") %> | Size: <%= doc.pageSize() %> Bytes
    </p>
